...
1.2 Set up views so we can test mutual relationships
	...
	- Make an 'add friend button on user show page' √
		- Use Jbuilder to set up ajax response for dynamic add.
...
2. Move logic from _notifications view to a controller or model in which the method gets called
	- "You should implement a `friend_requests_count` method in the `User` model. In that method, count the friend requests with `FriendRequest.where(friend: self)` (self will be the user on which the method is called). Now you can use `current_user.friend_requests_count'"
